/*
*Os effects reagem a uma ação, mas são executados de forma assíncrona. São parecidos com os reducers, mas são assíncronos
*/
...
@Injectable()//Pois ele é um serviço
export class HomeEffects {

  loadCurrentWeather$ = createEffect(() => this.actions$
    .pipe(
      ofType(fromHomeActions.loadCurrentWeather),//ofType: usado para filtrar as ações que queros lidar 
  ...
  loadCurrentWeatherById$ = createEffect(() => this.actions$
    .pipe(
      ...
      map((entity: CityWeather) => fromHomeActions.loadCurrentWeatherSuccess({entity})),//recebemos a action dentro do map e devolvemos qual a ação vai ser chamada
    )
  );
